...
"""
※ このスクリプトはスレッドセーフではない。
send_message_api.py を使うこと。

send_message_api の動作に問題なければ、このスクリプトは削除する。
...
from gsocial.management.tools.threadpool import ThreadPool
...
def get_callback_url(url_name=None):
    ...
    if not url_name:
        url_name = 'root_home'
    return reverse(url_name) + "?q=1" # q=1 Rexでつけてたので真似
...
class SendMessageTask(object):
    ...
    メッセージ送信
    """
    def __init__(self, osuser_ids, title_msg, body_msg, callback_url, progress_printer, progress):
    ...
    def run(self):
        ...
        message_api.sends(None, self.osuser_ids, self.title_msg, self.body_msg, self.callback_url)
        self.progress_printer.print_or_silent(self.progress)
...
def send_message_api_shard_osuser(title, body, logging_method=lambda x: None, only_shard=None, url_name=None):
    ...
    for shard in range(settings.HORIZONTAL_PARTITIONING_PARTITION_NUMBER):
        ...
        threadpool = ThreadPool(MAX_TREADS)
        progress = 0
        ...
        while True:
            ...
            task = SendMessageTask(
                list(userid_list), title, body,
                callback_url,
                progress_printer, progress
            )

            threadpool.add_job(task.run) # プールにつめる
        threadpool.start() #実行!
...
def send_message_api_listed_osuser(title, body, user_id_list, logging_method=lambda x: None, url_name=None):
    ...
    for i in xrange(0, all_count, MAX_SEND_COUNT):
        ...
        task = SendMessageTask(
            list(userid_list), title, body,
            callback_url,
            progress_printer, i + MAX_SEND_COUNT
        )
        threadpool.add_job(task.run) # プールにつめる
    threadpool.start() #実行!
    logging_method(u'finished.')
