...
class Command(BaseCommand):
    ...
    def handle(self, app=None, target=None, skip=False, merge=False, backwards=False, fake=False, db_dry_run=False, show_list=False, show_changes=False, database=DEFAULT_DB_ALIAS, delete_ghosts=False, ignore_ghosts=False, **options):
        ...
        #初期設定
        if not settings.HORIZONTALPARTITIONING_DBS:
            raise "settings.HORIZONTALPARTITIONING_DBS is None"

        if not settings.HORIZONTALPARTITIONING_APPS:
            raise "settings.HORIZONTALPARTITIONING_APPS is None"
        ...
        if not (show_list or show_changes):
            ...
            for db in settings.DATABASES:
                db_hp_check = db in settings.HORIZONTALPARTITIONING_DBS

                print '----------------------------------------'
                print '%s migrate HORIZONTALPARTITIONING(%s)' % (db, db_hp_check)
                print '----------------------------------------'

                for app in apps:
                    table_hp_check = app.app_label() in settings.HORIZONTALPARTITIONING_APPS

                    print '----------------------------------------'
                    print 'app_label %s HORIZONTALPARTITIONING(%s)' % (app.app_label(), table_hp_check)
                    print '----------------------------------------'

                    #水平分割DB and 水平分割Table　もしくは
                    #マスターDB and マスターTableの時のみmigrationを実行する
                    if db_hp_check  ==  table_hp_check:
