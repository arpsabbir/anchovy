...
class CssInclude(object):
    ...
    def apply(self, document):
        ...
        html_string = html.tostring(self.dom_xpath, encoding='utf-8')
        #strで入れるとUnicodeDecodeErrorが出る場合があるようなので、unicodeで入れてみる
        result = HTMLParser.HTMLParser().unescape(html_string.decode('utf-8','ignore'))
        # 一時退避させたものを復元
    ...
    def _changeCss(self, tag, style):
        ...
        if tag == 'hr' and self.agent in ('docomo', 'softbank'):
            ...
            if style.name == 'font-size':
                if style.value == '10px':
                    if self.agent in ('docomo', 'ezweb', 'au'):
